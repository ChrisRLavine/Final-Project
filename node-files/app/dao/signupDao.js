...
class SignupDao {
    ...
    updateById(req, res) {
        ...
        let sql = `UPDATE account SET ${fields.join('=?,')}=? WHERE id = ?`;
    ...
    create(req, res) {
        // Required Min Data
        if (!req.body.email || !req.body.username || !req.body.fname || !req.body.lname || !req.body.password) {
            res.json({
                "error": true,
        ...
        let sql = `INSERT INTO account (${fields.join(',')}) 
